...
router.get("/api/sales", async (req, res) => {
    try {
        ...
        const affiliateIds = Array.from(new Set(sales
        ...
        const nameByAffiliateId = new Map();
        for (const u of users) {
            if (u.affiliateId && u.fullName)
                nameByAffiliateId.set(u.affiliateId, u.fullName);
        }
        ...
        const mapped = sales.map((sale) => {
            ...
            const payoutDate = dates.length > 0
                ? new Date(Math.max(...dates.map((d) => d.getTime()))).toISOString()
                : null;
            const payoutStatuses = entries
                .map((e) => e.payout?.status ?? null)
                .filter((status) => status !== null);
            const payoutIds = Array.from(new Set(entries.map((e) => e.payoutId).filter((id) => typeof id === "string" && id.length > 0)));
            ...
            const payoutStatus = payoutStatuses.includes("paid") ? "paid" :
            ...
            const affiliateName = (affiliateId && nameByAffiliateId.get(affiliateId)) || affiliateId || "Unattributed";
            return {
                id: sale.id,
...
// ─────────────────────────────────────────────────────────────────────────────
router.post("/api/sales/:id/approve", async (req, res) => {
    try {
        const tenantId = (0, auth_1.getTenantId)(req);
