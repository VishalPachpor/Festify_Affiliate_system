...
function parseGoogleAuthIntent(value) {
 ...
 */
async function pickDefaultTenantId() {
    const tenant = await prisma_1.prisma.tenant.findFirst({ orderBy: { createdAt: "asc" } });
...
router.post("/api/auth/signup/organizer", async (req, res) => {
    try {
        const fullName = asNonEmptyString(req.body?.fullName);
        const emailRaw = asNonEmptyString(req.body?.email);
        const password = typeof req.body?.password === "string" ? req.body.password : "";
        ...
        if (!slug)
            slug = "event";
        const existingSlug = await prisma_1.prisma.tenant.findUnique({ where: { slug } });
        if (existingSlug) {
...
router.post("/api/auth/google", async (req, res) => {
    try {
        ...
        let user = (await prisma_1.prisma.user.findUnique({ where: { googleSub } })) ??
...
router.get("/api/auth/me", async (req, res) => {
    try {
        ...
        let decoded;
        try {
            decoded = jsonwebtoken_1.default.verify(header.slice(7), JWT_SECRET);
        }
        catch {
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/api/auth/google/redirect", (_req, res) => {
...
router.get("/api/auth/google/callback", async (req, res) => {
    ...
    const backendBase = process.env.PUBLIC_API_URL ?? "http://localhost:3001";
    const redirectUri = `${backendBase}/api/auth/google/callback`;
    function sendCallbackHtml(payload) {
...
// POST /api/auth/dev-token
// ─────────────────────────────────────────────────────────────────────────────
router.post("/api/auth/dev-token", (req, res) => {
    ...
    const token = (0, auth_1.generateDevToken)(tenantId, userId, role ?? "admin");
