...
exports.attributionRouter = void 0;
const express_1 = require("express");
const prisma_1 = require("../lib/prisma");
const auth_1 = require("../middleware/auth");
const cache_1 = require("../lib/cache");
const router = (0, express_1.Router)();
exports.attributionRouter = router;
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/api/attribution/summary", async (req, res) => {
    try {
        const tenantId = (0, auth_1.getTenantId)(req);
        const cacheKey = (0, cache_1.buildCacheKey)(tenantId, "attribution:summary");
        ...
        const [totalSales, attributedCount] = await Promise.all([
            prisma_1.prisma.sale.count({ where: { tenantId } }),
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/api/attribution/trends", async (req, res) => {
    try {
        const tenantId = (0, auth_1.getTenantId)(req);
        const DAYS = 14;
        const now = new Date();
        ...
        for (let i = DAYS - 1; i >= 0; i--) {
            ...
            const [daySales, dayAttributed] = await Promise.all([
                ...
                prisma_1.prisma.attributionClaim.count({
                    where: { tenantId, createdAt: { gte: dayStart, lte: dayEnd } },
                }),
            ]);
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/api/attribution/breakdown/source", async (req, res) => {
    try {
        const tenantId = (0, auth_1.getTenantId)(req);
        const claims = await prisma_1.prisma.attributionClaim.groupBy({
            by: ["method"],
            where: { tenantId },
            _count: true,
        });
        ...
        const items = await Promise.all(claims.map(async (claim) => {
            const revenueAgg = await prisma_1.prisma.sale.aggregate({
                where: {
                    tenantId,
        ...
        const totalSales = await prisma_1.prisma.sale.count({ where: { tenantId } });
        const unattributed = totalSales - totalAttributed;
        if (unattributed > 0) {
            const unattributedRevenue = await prisma_1.prisma.sale.aggregate({
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/api/attribution/breakdown/failures", async (req, res) => {
    try {
        const tenantId = (0, auth_1.getTenantId)(req);
        const [totalSales, attributedCount] = await Promise.all([
            prisma_1.prisma.sale.count({ where: { tenantId } }),
