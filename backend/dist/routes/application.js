...
router.get("/api/application/status", async (req, res) => {
    try {
        ...
        const authEmail = req.userId
            ? (await prisma_1.prisma.user.findUnique({ where: { id: req.userId } }))?.email?.toLowerCase() ?? null
            : null;
        const lookupEmail = authEmail ?? String(req.query.email ?? "").trim().toLowerCase();
        if (lookupEmail) {
...
router.post("/api/application/submit", async (req, res) => {
    try {
        ...
        const formEmail = applyingAs === "individual" ? email : contactPersonEmail;
