...
exports.milestonesRouter = void 0;
const express_1 = require("express");
const prisma_1 = require("../lib/prisma");
const auth_1 = require("../middleware/auth");
const router = (0, express_1.Router)();
exports.milestonesRouter = router;
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/api/milestones/tiers", async (req, res) => {
    try {
        ...
        let progressByMilestone = new Map();
        if (affiliateId) {
            const progress = await prisma_1.prisma.affiliateMilestoneProgress.findMany({
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/api/milestones/progress", async (req, res) => {
    try {
        const tenantId = (0, auth_1.getTenantId)(req);
        const affiliateId = req.affiliateId ?? null;
        const milestones = await prisma_1.prisma.milestone.findMany({
...
router.post("/api/milestones", async (req, res) => {
    try {
        ...
        const count = await prisma_1.prisma.milestone.count({ where: { tenantId } });
        let keyBase = slugifyKey(name) || "milestone";
        let key = keyBase;
        let suffix = 2;
        while (await prisma_1.prisma.milestone.findFirst({ where: { tenantId, key } })) {
            key = `${keyBase}-${suffix}`;
            suffix += 1;
        }
...
router.patch("/api/milestones/:id", async (req, res) => {
    try {
        if (!requireAdmin(req, res))
            return;
        const tenantId = (0, auth_1.getTenantId)(req);
        const id = String(req.params.id);
        const targetAmount = req.body?.targetAmount === undefined ? null : Number(req.body.targetAmount);
        const name = asNonEmptyString(req.body?.name);
        const description = asNonEmptyString(req.body?.description);
        const commissionRateBps = req.body?.commissionRateBps === undefined ? null : clampInt(req.body.commissionRateBps, 0, 10_000);
        const complimentaryTickets = req.body?.complimentaryTickets === undefined ? null : clampInt(req.body.complimentaryTickets, 0, 1_000);
        ...
        const existing = await prisma_1.prisma.milestone.findFirst({ where: { id, tenantId } });
        if (!existing) {
...
router.delete("/api/milestones/:id", async (req, res) => {
    try {
        ...
        const unlockedCount = await prisma_1.prisma.affiliateMilestoneProgress.count({
        ...
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.affiliateMilestoneProgress.deleteMany({
                where: { tenantId, milestoneId: id },
            }),
            prisma_1.prisma.milestone.delete({ where: { id } }),
        ]);
